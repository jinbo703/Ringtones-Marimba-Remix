...
// https://www.facebook.com/MarimbaRemixes/

#import "JLYoutubeActivity.h"

static NSString *YoutubeURLSchema = @"facebook://";
static NSString *YoutubeBaseURL = @"http://www.youtube.com/v/";
//static NSString *YoutubeBaseURL = @"http://apple.co/2xaSFSQ";
...
- (BOOL)isYouTubeURL:(NSURL*)url {
    if ([url isKindOfClass:[NSURL class]]) {
        NSDictionary *parameters = [self queryParametersForURL:url];
        if ([parameters[@"feature"] isEqualToString:@"Facebook"]) {
            return YES;
        }
    }
    return NO;
